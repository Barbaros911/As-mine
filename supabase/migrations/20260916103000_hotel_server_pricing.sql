...
update public.tarifs_partenaires t set validation=case t.destination_cle
  when 'cdg' then '{"terminal_prefix":"CDG"}'::jsonb
  when 'orly' then '{"terminal_prefix":"Orly"}'::jsonb
  when 'beauvais' then '{"terminal_prefix":"Beauvais"}'::jsonb
  when 'bourget' then '{"texte_contient":"Le Bourget"}'::jsonb
  when 'villepinte' then '{"texte_contient":"Villepinte"}'::jsonb
  when 'disney' then '{"texte_contient":"Disney"}'::jsonb
  when 'paris' then '{"verification_manuelle":true}'::jsonb
  else '{}'::jsonb end
from public.partenaires p
where p.id=t.partenaire_id and p.cle='easyhotel-aeroville' and t.actif;
...
begin
  ...
  if p_snapshot is not null then
    ...
    values(p_ref,(p_snapshot->>'prix_final_centimes')::integer,0,(p_snapshot->>'prix_final_centimes')::integer,null,(p_snapshot->>'montant_chauffeur_centimes')::integer,(p_snapshot->>'marge_ela_centimes')::integer,'tarification_serveur',true,true,now(),null);
    insert into public.evenements_reservation(course_ref,type_evenement,acteur_type,donnees)
      values(p_ref,'tarif_partenaire_verifie','systeme',jsonb_build_object('destination_cle',p_snapshot->>'destination_cle','prix_final_centimes',(p_snapshot->>'prix_final_centimes')::integer));
  end if;
  return p_ref;
end $$;
